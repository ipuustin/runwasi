...
fn test_external_hello_world() -> Result<(), Error> {
    ...
    let (output, retval) = common::run_wasmedge_test(&spec, &bytes)?;
    assert_eq!(retval, 0);
    assert!(output.starts_with("hello world"));

    let (output, retval) = common::run_wasmtime_test(&spec, &bytes)?;
    assert_eq!(retval, 0);
    assert!(output.starts_with("hello world"));
...
fn test_seccomp_hello_world_fail() -> Result<(), Error> {
    ...
    let (_, retval) = common::run_wasmedge_test(&spec, &bytes)?;
    assert_ne!(retval, 0);

    /*
    let (_, retval) = common::run_wasmtime_test(&spec, &bytes)?;
    assert_ne!(retval, 0);
    */
    Ok(())
}
